...
function CanvaApp() {
  ...
  const [apiKey, setApiKey] = useState(
    localStorage.getItem("urliApiKey") || "",
  );
  ...
  const shorten = async () => {
    if (!url || !apiKey) {
      setError("Enter a URL and your Urli API key");
      return;
    }
    setLoading(true);
    setError("");
    ...
    <Box padding="2u">
      <Rows spacing="2u">
        ...
        <Text size="small" tone="tertiary">
          Shorten links to use in your Canva designs
        </Text>
        ...
        <Text size="xsmall" tone="tertiary">
          Get your API key at urli.ideasprout.in/dashboard/api-keys
        </Text>
      </Rows>
    </Box>
  );
