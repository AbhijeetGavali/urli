...
const email = get('--email') || 'admin@urli.app'
...
async function main() {
  ...
  if (existing) {
    await prisma.user.update({ where: { email }, data: { role: 'ADMIN', plan: 'BUSINESS' } })
    console.log(`✅ Promoted ${email} to ADMIN`)
  } else {
    const passwordHash = await bcrypt.hash(password, 12)
    await prisma.user.create({
      data: { email, name, passwordHash, role: 'ADMIN', plan: 'BUSINESS', emailVerified: true },
    })
