...
export const subscriptionService = {
  ...
  createSubscription: async (userId: string, plan: "PRO" | "BUSINESS") => {
    ...
    if (
      existing?.razorpaySubId &&
      ["TRIALING", "ACTIVE"].includes(existing.status)
    ) {
      return { subscriptionId: existing.razorpaySubId, plan };
    }

    const trialEndsAt = new Date(Date.now() + 30 * 86400_000);

    const sub = await razorpay.subscriptions.create({
      plan_id: planId,
      total_count: 12,
      quantity: 1,
      // start_at defers the first billing charge by 30 days (free trial)
      // Razorpay does a ₹0/mandate auth at signup; actual plan charge starts at start_at
  ...
  verifyPayment: async (
    ...
    const sub = await subscriptionRepo.findByRazorpayId(
      data.razorpay_subscription_id,
    );
    if (!sub) throw new AppError("Subscription not found", 404);

    // Auth payment succeeded - subscription is now TRIALING (start_at is 30 days away)
    // Grant the plan immediately so user gets full access during the trial
    await subscriptionRepo.update(sub.id, { status: "TRIALING" });
    await userRepo.update(userId, { plan: sub.plan });
    return { ok: true, plan: sub.plan, trialEndsAt: sub.trialEndsAt };
  },
