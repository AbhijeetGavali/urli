...
const APP_HOST = process.env.SHORT_DOMAIN || 'urli.app'

export const domainService = {
  list: (userId: string) => domainRepo.findByUser(userId),

  add: async (userId: string, plan: string, domain: string) => {
    if (plan === 'FREE') throw new AppError('Custom domains require Pro plan', 403)
    const existing = await domainRepo.findByDomain(domain)
    if (existing && existing.userId !== userId) throw new AppError('Domain already registered', 409)
  ...
  verify: async (userId: string, domainId: string) => {
    ...
    if (!record || record.userId !== userId) throw new AppError('Domain not found', 404)
    ...
    try {
      ...
      throw new AppError('DNS lookup failed. Check your domain configuration.', 400)
    }
  },
