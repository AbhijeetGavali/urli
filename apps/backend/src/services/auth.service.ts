...
export const authService = {
  register: async (data: { email: string; name: string; password: string; termsAccepted: boolean }) => {
    if (await userRepo.findByEmail(data.email)) throw new AppError('Email already registered', 409)
    const passwordHash = await bcrypt.hash(data.password, 12)
    const verifyToken = nanoid(32)
    const now = new Date()
    const user = await userRepo.create({
      email: data.email, name: data.name, passwordHash,
      plan: 'FREE', trialEndsAt: trialEnd(), verifyToken,
  ...
  googleAuth: async (data: { googleId: string; email: string; name: string; avatar?: string }) => {
    let user = await userRepo.findByGoogleId(data.googleId) || await userRepo.findByEmail(data.email)
    if (!user) {
      user = await userRepo.create({ ...data, plan: 'FREE', trialEndsAt: trialEnd(), emailVerified: true })
      await mailer.sendWelcome(user.email, user.name).catch(() => {})
    } else if (!user.googleId) {
      user = await userRepo.update(user.id, { googleId: data.googleId, avatar: data.avatar })
    }
    return sanitize(user)
  },
  ...
  resetPassword: async (token: string, password: string) => {
    ...
    await userRepo.update(user.id, { passwordHash, resetToken: null, resetTokenExpiry: null })
  },
  ...
  deleteAccount: async (id: string, password?: string) => {
    ...
    if (user.passwordHash && password) {
      if (!await bcrypt.compare(password, user.passwordHash)) throw new AppError('Incorrect password', 401)
    }
  ...
  exportData: async (id: string) => {
    ...
    if (!user) throw new AppError('User not found', 404)
    const { passwordHash, verifyToken, resetToken, resetTokenExpiry, ...safeUser } = user as any
    return safeUser
  },
}

function sanitize(user: any) {
  const { passwordHash, verifyToken, resetToken, resetTokenExpiry, ...rest } = user
  return rest
}
