...
export async function redirectRoute(app: FastifyInstance) {
  app.get('/:slug', {
    ...
    if (cached) {
      ...
      if (link) await redis.setex(linkCacheKey(slug), LINK_CACHE_TTL, JSON.stringify(link))
    }

    if (!link || !link.isActive) return reply.code(404).send({ error: 'Link not found' })

    // Expiry check
    if (link.expiresAt && new Date(link.expiresAt) < new Date()) {
      return reply.redirect(302, `${process.env.FRONTEND_URL}/expired?slug=${slug}`)
    }
    ...
    let country = '', city = ''
    try {
      const geo = await fetch(`http://ip-api.com/json/${ip}?fields=country,city,status`).then(r => r.json()) as any
    ...
    if (link.smartRules?.length) {
      for (const rule of link.smartRules) {
        if (rule.type === 'device' && rule.value === device) { destination = rule.destination; break }
        if (rule.type === 'country' && rule.value === country) { destination = rule.destination; break }
        if (rule.type === 'os' && rule.value === os) { destination = rule.destination; break }
      }
    }
    ...
    setImmediate(async () => {
      try {
        await Promise.all([
          clickRepo.create({ linkId: link.id, userId: link.userId, ip, country, city, device, browser, os, referrer }),
    ...
    if (link.pixelIds?.length) {
      ...
      if (pixels.length) {
        const scripts = pixels.map((p: any) => {
          if (p.type === 'FACEBOOK') return `<script>!function(f,b,e,v,n,t,s){if(f.fbq)return;n=f.fbq=function(){n.callMethod?n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';n.queue=[];t=b.createElement(e);t.async=!0;t.src=v;s=b.getElementsByTagName(e)[0];s.parentNode.insertBefore(t,s)}(window,document,'script','https://connect.facebook.net/en_US/fbevents.js');fbq('init','${p.pixelId}');fbq('track','PageView');</script>`
          if (p.type === 'GOOGLE') return `<script async src="https://www.googletagmanager.com/gtag/js?id=${p.pixelId}"></script><script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${p.pixelId}');</script>`
          return ''
        }).join('')
