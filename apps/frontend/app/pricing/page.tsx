...
import { useCreateSubscriptionMutation, useVerifyPaymentMutation } from '../../store/api/miscApi'
...
const PLANS = [
  {
    ...
    features: ['50 short links / month', 'Basic click tracking', 'QR code generation', '30-day analytics history'],
    notIncluded: ['Retargeting pixels', 'Link expiry', 'UTM templates', 'Link-in-bio page', 'API access'],
...
const FAQ = [
  { q: 'Is the 30-day trial really free?', a: 'Yes — no credit card required. You get full Pro features for 30 days. After that, subscribe or drop to the free plan automatically.' },
  { q: 'What happens to my links if I cancel?', a: 'Your short links keep working forever. You lose access to Pro features (analytics beyond 30 days, pixels, etc.) but nothing breaks for your visitors.' },
  { q: 'Is Founder\'s Pricing really permanent?', a: 'Yes. Subscribe at ₹999/mo or ₹2,999/mo today and that rate is yours forever — even when we raise prices for new customers.' },
  { q: 'Can I upgrade or downgrade later?', a: 'Yes, anytime. Upgrades take effect immediately. Downgrades apply at the end of your current billing period.' },
  { q: 'Do you offer refunds?', a: '7-day no-questions-asked refund on your first payment. Email support@ideasprout.in and we\'ll process it same day.' },
]
  ...
  const handleSubscribe = async (planId: string) => {
    ...
    try {
      ...
      await initiatePayment({
        subscriptionId, plan: planId, user,
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-gray-50 border-b border-gray-100 py-14 text-center">
        <div className="max-w-2xl mx-auto px-5">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-3 tracking-tight">Simple, honest pricing</h1>
          <p className="text-gray-500">No tricks. No ads on your links. 30-day free trial on all paid plans.</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-5 py-14">
        <div className="grid md:grid-cols-3 gap-5 items-stretch">
          {PLANS.map((plan) => {
            ...
            return (
              <div key={plan.id} className={`relative flex flex-col rounded-2xl border ${isPro ? 'border-blue-500 ring-1 ring-blue-500/20 shadow-xl shadow-blue-100/50' : 'border-gray-200'} bg-white overflow-hidden`}>
                {/* Top accent for Pro */}
                {isPro && <div className="h-1 bg-gradient-to-r from-blue-500 to-indigo-500" />}

                <div className="p-6 flex flex-col flex-1">
                  {/* Header */}
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{plan.name}</div>
                  ...
                  {plan.price > 0 && (
                    <div className="flex items-center gap-1.5 mb-4">
                      ...
                      <span className="text-xs text-green-600 font-medium">30-day free trial included</span>
                    </div>
                  )}
                  ...
                  <ul className="space-y-2 flex-1 mb-6">
                    {plan.features.map(f => (
                      <li key={f} className="flex items-start gap-2 text-sm text-gray-700">
                        <Check size={14} className={`shrink-0 mt-0.5 ${isPro ? 'text-blue-500' : 'text-green-500'}`} strokeWidth={2.5} />
                        {f}
                      </li>
                    ))}
                    {plan.notIncluded.map(f => (
                      <li key={f} className="flex items-start gap-2 text-sm text-gray-400">
                        <span className="shrink-0 mt-0.5 w-3.5 h-3.5 flex items-center justify-center text-gray-300 text-xs">—</span>
                        {f}
                      </li>
                    ))}
                  </ul>
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 rounded-xl font-semibold text-sm transition-all active:scale-[0.98] disabled:opacity-50 ${
                      isCurrent ? 'bg-gray-100 text-gray-400 cursor-default' : plan.ctaStyle
                    }`}>
                    {isCurrent ? '✓ Current plan' : plan.cta}
                  </button>

                  {plan.price > 0 && !isCurrent && (
                    <p className="text-center text-[11px] text-gray-400 mt-2">No credit card required</p>
                  )}
                </div>
              </div>
            )
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-500 w-1/2">Feature</th>
                  <th className="px-4 py-3.5 text-xs font-semibold text-gray-400 text-center">Starter</th>
                  <th className="px-4 py-3.5 text-xs font-bold text-blue-700 text-center bg-blue-50/50">Pro</th>
                  <th className="px-4 py-3.5 text-xs font-semibold text-gray-500 text-center">Business</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {[
                  ...
                  <tr key={String(feat)} className={i % 2 ? 'bg-gray-50/30' : ''}>
                    <td className="px-5 py-3 text-gray-700 text-xs font-medium">{feat}</td>
                    {[free, pro, biz].map((val, j) => (
                      <td key={j} className={`px-4 py-3 text-center ${j === 1 ? 'bg-blue-50/20' : ''}`}>
                        {typeof val === 'boolean'
                          ? val
                            ? <Check size={14} className="text-green-500 mx-auto" strokeWidth={2.5} />
      ...
      <div className="py-14">
        <div className="max-w-2xl mx-auto px-5">
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            More questions? <a href="mailto:support@ideasprout.in" className="text-blue-600 hover:underline">Email us</a> — we reply within 24 hours.
          </p>
        </div>
      </div>
      ...
      <div className="border-t border-gray-100 bg-gray-50 py-14 text-center">
        <div className="max-w-lg mx-auto px-5">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Start your free trial today</h2>
          <p className="text-gray-500 text-sm mb-6">Founder's pricing — ₹999/mo locked in forever when you subscribe now.</p>
          <Link href="/register" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm px-8 py-3.5 rounded-xl transition-colors shadow-sm">
