...
import { formatDate, shortUrl, copyToClipboard, getLinkStatus, STATUS_COLORS } from './utils'
...
export default function DashboardPage() {
  ...
  const { data, isLoading } = useGetLinksQuery({ page, limit: 20, search: search || undefined })
          ...
          <>
            <div className="divide-y divide-gray-50">
              {data?.links?.map((link: any) => {
                ...
                return (
                  <div key={link.id} className="p-4 flex items-start gap-4 hover:bg-gray-50/50 transition-colors group">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <a href={`https://${shortUrl(link.slug)}`} target="_blank" rel="noopener noreferrer"
                        ...
                        {link.title && <span className="text-xs text-gray-400 truncate max-w-[200px]">{link.title}</span>}
                      </div>
                      <div className="text-xs text-gray-400 truncate max-w-sm">{link.originalUrl}</div>
                      <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-400">
                        <span>{formatDate(link.createdAt)}</span>
                        <span>·</span>
                        <span className="font-medium text-gray-600">{link.clickCount} clicks</span>
            ...
            {data?.total > 20 && (
              <div className="p-4 border-t border-gray-100 flex justify-between items-center">
                ...
                <span className="text-sm text-gray-400">Page {page} · {data.total} total</span>
                <button onClick={() => setPage(p => p + 1)} disabled={data?.links?.length < 20}
                  className="text-sm text-gray-500 hover:text-gray-700 disabled:opacity-40 px-3 py-1.5 rounded-lg hover:bg-gray-100">
                  Next →
                </button>
              </div>
