...
import { useGetUTMTemplatesQuery, useGetPixelsQuery } from '../store/api/miscApi'
...
export function CreateLinkModal({ onClose, onCreated }: Props) {
  ...
  const applyTemplate = (t: any) => {
    setForm(f => ({ ...f, utmSource: t.source || '', utmMedium: t.medium || '', utmCampaign: t.campaign || '', utmTerm: t.term || '', utmContent: t.content || '' }))
    setTab('UTM')
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch(showToast({ message: err?.data?.error || 'Failed to create link', type: 'error' }))
    }
  }
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-4">
            {tab === 'Basic' && (
              <>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Custom slug (optional)</label>
                  ...
                  <p className="text-xs text-gray-400 mt-1">Leave blank to auto-generate. Letters, numbers, hyphens only.</p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Title (optional)</label>
            ...
            {tab === 'UTM' && (
              <>
                {utmData?.templates?.length > 0 && (
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Apply template</label>
                ...
                {[
                  { key: 'utmSource', label: 'Source', placeholder: 'e.g. instagram' },
                  { key: 'utmMedium', label: 'Medium', placeholder: 'e.g. social' },
                  { key: 'utmCampaign', label: 'Campaign', placeholder: 'e.g. summer-sale-2025' },
                  { key: 'utmTerm', label: 'Term', placeholder: 'e.g. url shortener' },
                  { key: 'utmContent', label: 'Content', placeholder: 'e.g. hero-button' },
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="number" min="1" value={form.maxClicks} onChange={e => set('maxClicks', e.target.value)}
                ...
                {user?.plan !== 'FREE' && pixelData?.pixels?.length > 0 && (
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Retargeting pixels</label>
                    <div className="space-y-2">
                      {pixelData.pixels.map((p: any) => (
                        <label key={p.id} className="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50">
                          <input type="checkbox" checked={form.pixelIds.includes(p.id)} onChange={() => togglePixel(p.id)}
                ...
                {user?.plan === 'FREE' && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
                    <strong>Pro features:</strong> Link expiry and retargeting pixels require a Pro plan.
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
            <button type="button" onClick={onClose} className="flex-1 border border-gray-200 text-gray-700 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50">
              Cancel
            </button>
            <button type="submit" disabled={isLoading || !form.originalUrl}
