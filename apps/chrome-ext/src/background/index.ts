const API = "https://api.urli.ideasprout.in"; // configurable
...
chrome.contextMenus.onClicked.addListener(async (info) => {
  ...
  if (!token) {
    chrome.tabs.create({ url: "https://urli.ideasprout.in/login" });
    return;
  }
  ...
  try {
    ...
    chrome.notifications.create({
      ...
      iconUrl: "icons/icon48.png",
      title: "Urli error",
      message: "Failed to shorten link. Check your connection.",
    });
  }
});
