...
function Popup() {
  ...
  useEffect(() => {
    chrome.storage.local.get('accessToken', (r) => setToken(r.accessToken || null))
  ...
  return (
    <div className="p-4 w-80">
      ...
      <div className="space-y-3">
        <input value={url} onChange={e => setUrl(e.target.value)} placeholder="URL to shorten…"
        ...
        {shortUrl && (
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-blue-600 flex-1 truncate">{shortUrl}</span>
              <button onClick={copy} className="text-xs bg-white border border-gray-200 px-2 py-1 rounded hover:bg-gray-50">
                {copied ? '✓' : 'Copy'}
              </button>
            </div>
          </div>
